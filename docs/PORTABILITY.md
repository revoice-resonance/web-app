...
**注意**：项目已移除 Supabase 依赖，不再需要相关环境变量。
...
### Edge Functions
在 Cloudflare Workers 的 secrets 管理界面配置：
...
### 方案 B: Vercel / Netlify + Cloudflare Workers
1. 静态资源部署到 Vercel / Netlify。
2. Edge Functions 部署到 Cloudflare Workers。
3. 前端调用路径保持相对 `/api/*`，指向 Cloudflare Workers 域名。
...
## 已知例外
- `miniprogram/` 微信小程序代码（小程序无环境变量机制，待用户决定独立处理方案）。
