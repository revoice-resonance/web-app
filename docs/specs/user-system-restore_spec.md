...
CloudSpeech 迁移后有两个遗留问题：(1) 训练/词表页面标记为"实验功能"不可用；(2) 无用户标识，数据无法跨设备持久化。本次构建基于手机号+SMS验证码的用户系统（PostgreSQL 存储，Worker 通过 VPC 访问），并将训练功能提升为正式导航。
...
- AC3.5: Worker `POST /api/asr/recognize` 接收可选 `phrase_hints` 字段，透传给 CloudSpeech ASR（若 API 支持）

**Note:** 若 CloudSpeech ASR 不支持 phrase hints，`phraseHints` 参数保留为 no-op，但前端管线已就位。训练数据（短语+录音）保持在 localStorage——不收集到后端。
...
│  ├─────────────────────────────────────────┤     │
│  │ Cloud: POST /api/asr/recognize           │ ◄── CloudSpeech ASR │
│  │        GET  /api/asr/health              │     │
│  │        POST /api/tts/speak               │ ◄── CloudSpeech TTS │
